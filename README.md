...
`deliverEvent(event, send, { maxRetries })` calls an injected async transport returning `{ status }`. The default `maxRetries` is **5 retries after the initial attempt**, for **6 total attempts**. HTTP 2xx succeeds immediately. HTTP 408, HTTP 429, HTTP 5xx, and thrown transport errors are retried immediately until the budget is exhausted. Other HTTP statuses stop immediately. The result is `{ delivered, attempts, status }`; transport errors use `status: null`.
